...
fn main() {
    ...
    //game loop with initializations
...
impl WindowHandler<String> for MyWindowHandler {
    //code for when a screen is drawn
    ...
    //code for when the mouse button is clicked
    fn on_mouse_button_down(&mut self, helper: &mut WindowHelper<String>, button: MouseButton) {
        if button == MouseButton::Left {
            self.mouse_button_down = true;
        }

        //checks to see if the click was in the boundaries of a button:
    ...
    //code that updates self.mouse_pos when the mouse is moved
...
impl MyWindowHandler {
    //reset the actual language and the languages associated with the 4 buttons
    fn reset_lang(&mut self) {
        ...
        let t_new = get_text(&self.actual_lang.0);
    ...
    //when the color mouse is activated (click bottom-right button on main screen)
    //this is code to generate the random rgb values
...
//cdoe that returns random text for appropriate language abbreviations
...
//vector of tuples matching appropriate language abbreviations with language names
